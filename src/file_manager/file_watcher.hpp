...
namespace wtr
{
    ...
    first event, except for "die" instead of "live". */
      ...
      events and will return nothing. */
			...
			const long long effect_time{std::chrono::duration_cast<Nanos>(TimePoint{Clock::now()}.time_since_epoch()).count()};

			const std::unique_ptr<event> associated{nullptr};

			inline event(const event& from) noexcept :
			    path_name{from.path_name},
			    effect_type{from.effect_type},
			    path_type{from.path_type},
			    effect_time{from.effect_time},
			    associated{from.associated ? std::make_unique<event>(*from.associated) : nullptr} {};

			inline event(const std::filesystem::path& path_name, enum effect_type effect_type, enum path_type path_type) noexcept :
			    path_name{path_name},
			    effect_type{effect_type},
			    path_type{path_type} {};

			inline event(const event& base, event&& associated) noexcept :
      ...
      because the `effect_time` is typically (not always) unique. */
			inline friend auto operator==(const event& l, const event& r) noexcept -> bool
			{
				return l.path_name == r.path_name && l.effect_time == r.effect_time && l.path_type == r.path_type && l.effect_type == r.effect_type && (l.associated && r.associated ? *l.associated == *r.associated : !l.associated && !r.associated);
			}
...
*/
	...
	public:
		...
		const int fd = eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK | EFD_SEMAPHORE);
		...
		inline auto release() noexcept -> state
		{
			...
			auto was_exchanged = this->is.compare_exchange_strong(exchange_when, released, std::memory_order_release, std::memory_order_acquire);
		...
		inline auto release() noexcept -> enum state
		{
			return this->is = released;
		}

		inline auto state() const noexcept -> enum state
		{
			return this->is;
		}
...
inline constexpr unsigned fsev_flag_effect_any
  ...
  | fsev_flag_effect_rename;
					...
					struct ContextData
					{
						...
						const ::wtr::watcher::event::callback& callback{};
...
*/
					inline auto path_from_event_at(void* event_recv_paths, unsigned long i) -> std::filesystem::path
					{
						if (event_recv_paths)
						{
							if (const void* from_arr = CFArrayGetValueAtIndex(static_cast<CFArrayRef>(event_recv_paths), static_cast<CFIndex>(i)))
							{
								if (const void* from_dict = CFDictionaryGetValue(static_cast<CFDictionaryRef>(from_arr), kFSEventStreamEventExtendedDataPathKey))
								{
									if (const char* as_cstr = CFStringGetCStringPtr(static_cast<CFStringRef>(from_dict), kCFStringEncodingUTF8))
									{
										return {as_cstr};
									}
								}
					...
					inline auto event_recv_one(ContextData& ctx, const std::filesystem::path& path, unsigned flags)
					{
    ...
    */
    ...
    set of paths which we've seen created. */
					...
					inline auto event_recv(ConstFSEventStreamRef,      /*  `ConstFS..` is important */
					                       ...
					                       const unsigned* flags,      /*  Event flags */
					                       const FSEventStreamEventId* /*  A unique stream id */
					                       ) -> void
					{
					...
					inline auto open_event_stream(const std::filesystem::path& path, dispatch_queue_t queue, void* ctx) -> FSEventStreamRef
					{
      ...
      rule), but we don't appear to. */
						...
						const void* path_cfstring = CFStringCreateWithCString(nullptr, path.c_str(), kCFStringEncodingUTF8);

						/*  A predefined structure which is (from CFArray.h) --
      "appropriate when the values in a CFArray are CFTypes" */
						static const auto cf_arr_ty = kCFTypeArrayCallBacks;

						CFArrayRef path_array = CFArrayCreate(nullptr,        /*  A custom allocator is optional */
      ...
      the kernel sees for the paths in `path_array`. */
						...
						auto stream = FSEventStreamCreate(nullptr,           /*  A custom allocator is optional */
					...
					inline auto wait(const semabin& sb)
					{
						auto s = sb.state();
    ...
    we'll do that.
*/
					inline auto close_event_stream(FSEventStreamRef stream, ContextData& ctx) -> bool
					{
						if (!stream)
						{
							return false;
    ...
    in the `/etc/wip-fsevents-issue` directory.  */
				inline auto watch(const std::filesystem::path& path, const ::wtr::watcher::event::callback& cb, const semabin& is_living) -> bool
				{
...
#if (defined(__linux__) || __ANDROID_API__) && !defined(WATER_WATCHER_USE_WARTHOG)
...
namespace detail::wtr::watcher::adapter
{
  ...
  */
  ...
  //case result::w_sys_noent:                        return "w/sys/noent@";
	...
	inline auto send_msg = [](result r, auto path, const auto& cb)
	{
	...
	inline auto make_ep = [](int ev_fs_fd, int ev_il_fd) -> ep
	{
		...
		bool ctl_ok = fd >= 0 && epoll_ctl(fd, EPOLL_CTL_ADD, ev_fs_fd, &want_ev_fs) >= 0 && epoll_ctl(fd, EPOLL_CTL_ADD, ev_il_fd, &want_ev_il) >= 0;
		if (!ctl_ok && fd >= 0)
			close(fd), fd = -1;
		return ep{.fd = fd};
	};

	inline auto is_dir(const char* const path) -> bool
	{
		struct stat s;
		return stat(path, &s) == 0 && S_ISDIR(s.st_mode);
	}

	inline auto strany = [](const char* const s, auto... cmp) -> bool
	{
...
*/
	template<class Fn>
	inline auto walkdir_do(const char* const path, const Fn& f) -> void
	{
...
struct ke_fa_ev {
  ...
  alignas(fanotify_event_metadata) char buf[buf_len]{0};
	...
	inline auto do_mark = [](const char* const dirpath, int fa_fd, const auto& cb) -> result
	{
		auto e = result::w_sys_not_watched;
		char real[PATH_MAX];
		int anonymous_wd = realpath(dirpath, real) && is_dir(real) ? fanotify_mark(fa_fd, FAN_MARK_ADD, ke_fa_ev::recv_flags, AT_FDCWD, real) : -1;
		if (anonymous_wd == 0)
		{
	...
	inline auto make_sysres = [](const char* const base_path, const auto& cb, const semabin& is_living) -> sysres
	{
		...
		walkdir_do(base_path,
		           [&](auto dir)
		           {
			           do_mark(dir, fa_fd, cb);
		           });
    ...
    Confusing, right? */
	inline auto pathof(const fanotify_event_metadata* const mtd) -> std::string
	{
      ...
      can start writing the file name after its offset. */
		{
			if (file_name_offset > 0)
			{
				...
				if (file_name && not_selfdir)
				{
					snprintf(beg, end, "/%s", file_name);
				}
			}
		}
	...
	inline auto peek(const fanotify_event_metadata* const m, size_t read_len) -> const fanotify_event_metadata*
	{
		if (m)
		{
	...
	struct Parsed
	{
		...
		const fanotify_event_metadata* next = nullptr;
		unsigned this_len                   = 0;
	};

	inline auto parse_ev(const fanotify_event_metadata* const m, size_t read_len) -> Parsed
	{
		...
		auto e = [et, pt](auto*... m) -> ev
		{
			return ev(ev(pathof(m), et, pt)...);
		};
		auto one = [&](auto* m) -> Parsed
		{
			return {e(m), n, m->event_len};
		};
	...
	inline auto do_mark_if_newdir = [](const ::wtr::watcher::event& ev, int fa_fd, const auto& cb) -> result
	{
		auto is_newdir = ev.effect_type == ::wtr::watcher::event::effect_type::create && ev.path_type == ::wtr::watcher::event::path_type::dir;
		if (is_newdir)
		{
	...
	inline auto do_ev_recv = [](const auto& cb, sysres& sr) -> result
	{
...
struct ke_in_ev {
  ...
  */
  ...
  static_assert(sizeof(ev_buf) % sizeof(inotify_event) == 0, "alignment");
	...
	inline auto do_mark = [](const char* const dirpath, int dirfd, auto& dm, const auto& cb) -> result
	{
		auto e = result::w_sys_not_watched;
		char real[PATH_MAX];
		int wd = realpath(dirpath, real) && is_dir(real) ? inotify_add_watch(dirfd, real, ke_in_ev::recv_mask) : -1;
		if (wd > 0)
		{
	...
	inline auto make_sysres = [](const char* const base_path, const auto& cb, const semabin& is_living) -> sysres
	{
		...
		auto make_dm = [&](result* ok, int in_fd) -> ke_in_ev::paths
		{
			...
			walkdir_do(base_path,
			           [&](auto dir)
			           {
				           do_mark(dir, in_fd, dm, cb);
			           });
			if (dm.empty())
			{
				*ok = result::e_self_noent;
			}
			return dm;
		};
	...
	inline auto peek(const inotify_event* const in_ev, const inotify_event* const ev_tail) -> inotify_event*
	{
	...
	inline auto parse_ev(const std::filesystem::path& dirname, const inotify_event* const in, const inotify_event* const tail) -> parsed
	{
		using ev    = ::wtr::watcher::event;
		using ev_pt = enum ev::path_type;
		using ev_et = enum ev::effect_type;
		auto pathof = [&](const inotify_event* const m)
		{
...
namespace detail::wtr::watcher::adapter
{
	...
	inline auto watch = [](const auto& path, const auto& cb, const auto& is_living) -> bool
	{
		auto platform_watch = [&](auto make_sysres, auto do_ev_recv) -> result
		{
			...
			{
				int ep_c = epoll_wait(sr.ep.fd, sr.ep.interests, sr.ep.q_ulim, sr.ep.wake_ms);
				if (ep_c < 0)
				{
...
namespace detail
{
    ...
    so 65536 might also work well. */
					...
					public:
						...
						watch_event_proxy(const std::filesystem::path& path) noexcept :
						    path{path}
						{
							memcpy(path_name, path.c_str(), path.string().size());

							path_handle = CreateFileW(path.c_str(), FILE_LIST_DIRECTORY, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, nullptr, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OVERLAPPED, nullptr);

							if (path_handle)
							{
								event_completion_token = CreateIoCompletionPort(INVALID_HANDLE_VALUE, nullptr, 0, 0);
							}

							if (event_completion_token)
							{
								is_valid = CreateIoCompletionPort(path_handle, event_completion_token, (ULONG_PTR)path_handle, 1) && ResetEvent(event_token);
							}
						}
					...
					inline auto do_event_recv(watch_event_proxy& w, const ::wtr::watcher::event::callback& callback) noexcept -> bool
					{
						...
						auto read_ok = ReadDirectoryChangesW(w.path_handle, w.event_buf, event_buf_len_max, true, FILE_NOTIFY_CHANGE_SECURITY | FILE_NOTIFY_CHANGE_CREATION | FILE_NOTIFY_CHANGE_LAST_ACCESS | FILE_NOTIFY_CHANGE_LAST_WRITE | FILE_NOTIFY_CHANGE_SIZE | FILE_NOTIFY_CHANGE_ATTRIBUTES | FILE_NOTIFY_CHANGE_DIR_NAME | FILE_NOTIFY_CHANGE_FILE_NAME, &bytes_returned, &w.event_overlap, nullptr);
						...
						else
						{
							...
							case ERROR_IO_PENDING:
								w.event_buf_len_ready = 0;
								w.is_valid            = false;
								callback({"e/sys/read/pending", ::wtr::event::effect_type::other, ::wtr::event::path_type::watcher});
								break;
							default:
								callback({"e/sys/read", ::wtr::event::effect_type::other, ::wtr::event::path_type::watcher});
								break;
							}
							return false;
						}
					...
					inline auto do_event_send(watch_event_proxy& w, const ::wtr::watcher::event::callback& callback) noexcept -> bool
					{
						using namespace ::wtr::watcher;
						...
						if (is_valid(w))
						{
							while (buf + sizeof(FILE_NOTIFY_INFORMATION) <= buf + w.event_buf_len_ready)
							{
								...
								{
									auto path_name = w.path / std::wstring{buf->FileName, buf->FileNameLength / 2};
									...
									auto path_type = [&path_name]()
									{
										try
										{
											return std::filesystem::is_directory(path_name) ? event::path_type::dir : event::path_type::file;
										}
										catch (...)
										{
    ...
    return when dead
    true if no errors */
				inline auto watch(const std::filesystem::path& path, const ::wtr::watcher::event::callback& callback, const semabin& is_living) noexcept -> bool
				{
					using namespace ::wtr::watcher;
					...
					if (is_valid(w))
					{
						...
						while (is_living.state() == semabin::state::pending)
						{
							...
							bool complete = GetQueuedCompletionStatus(w.event_completion_token, &w.event_buf_len_ready, &completion_key, &overlap, delay_ms_dw);
...
namespace detail
{
		...
		namespace watcher
		{
			namespace adapter
			{
				namespace
				{
					...
					inline constexpr std::filesystem::directory_options scan_dir_options = std::filesystem::directory_options::skip_permission_denied & std::filesystem::directory_options::follow_directory_symlink;
    ...
    - Returns false if the file tree cannot be scanned. */
					inline bool scan(const std::filesystem::path& path, const auto& send_event, bucket_type& bucket) noexcept
					{
						/*  - Scans a (single) file for changes.
      - Updates our bucket to match the changes.
      - Calls `send_event` when changes happen.
      - Returns false if the file cannot be scanned. */
						const auto& scan_file = [&](const std::filesystem::path& file, const auto& send_event) -> bool
						{
							using namespace ::wtr::watcher;
							using std::filesystem::exists, std::filesystem::is_regular_file, std::filesystem::last_write_time;
            ...
            and remove it from the bucket. */
									send_event(event{file, event::effect_type::destroy, event::path_type::file});
            ...
            indicating creation. */
									...
									send_event(event{file, event::effect_type::create, event::path_type::file});
								}
              ...
              indicating modification */
										send_event(event{file, event::effect_type::modify, event::path_type::file});
									}
								}
								return true;
      ...
      - Returns false if the directory cannot be scanned. */
						const auto& scan_directory = [&](const std::filesystem::path& dir, const auto& send_event) -> bool
						{
							using std::filesystem::recursive_directory_iterator, std::filesystem::is_directory;
							/*  if this thing is a directory */
							if (is_directory(dir))
							{
								/*  try to iterate through its contents */
								auto dir_it_ec = std::error_code{};
								for (auto const& file : recursive_directory_iterator(dir, scan_dir_options, dir_it_ec))
								{
						...
						return scan_directory(path, send_event) ? true : scan_file(path, send_event) ? true : false;
					};

					/*  If the bucket is empty, try to populate it.
    otherwise, prune it. */
					inline bool tend_bucket(const std::filesystem::path& path, const auto& send_event, bucket_type& bucket) noexcept
					{
						/*  Creates a file map, the "bucket", from `path`. */
						const auto& populate = [&](const std::filesystem::path& path) -> bool
						{
							using std::filesystem::exists, std::filesystem::is_directory, std::filesystem::recursive_directory_iterator, std::filesystem::last_write_time;
        ...
        there is nothing to do here; we prune later. */
							...
							if (exists(path))
							{
								...
								{
									for (auto const& file : recursive_directory_iterator(path, scan_dir_options, dir_it_ec))
									{
										if (!dir_it_ec)
										{
						...
						const auto& prune = [&](const std::filesystem::path& path, const auto& send_event) -> bool
						{
							using namespace ::wtr::watcher;
							using std::filesystem::exists, std::filesystem::is_regular_file, std::filesystem::is_directory, std::filesystem::is_symlink;
            ...
            and remove it from our bucket. */
						...
						return bucket.empty() ? populate(path) ? true : prune(path, send_event) ? true : false : true;
					};
...
namespace wtr
{
    ...
    Happy hacking. */
		...
		public:
			inline watch(const std::filesystem::path& path, const event::callback& callback) noexcept :
			    watching{std::async(std::launch::async,
			                        [this, path, callback]
			                        {
				                        ...
				                        auto pre_ok =
				                            !ec && std::filesystem::is_directory(abs_path, ec) && !ec && this->is_living.state() == sb::state::pending;

				                        auto live_msg = (pre_ok ? "s/self/live@" : "e/self/live@") + abs_path.string();
				                        callback({live_msg, event::effect_type::create, event::path_type::watcher});

				                        auto post_ok = !pre_ok || watch(abs_path, callback, this->is_living);

				                        auto die_msg = (post_ok ? "s/self/die@" : "e/self/die@") + abs_path.string();
				                        callback({die_msg, event::effect_type::destroy, event::path_type::watcher});
			...
			{
				return this->is_living.release() != sb::state::pending && this->watching.valid() && this->watching.get();
			};

			inline ~watch() noexcept
			{
				this->close();
			}
		};
