...
static std::map<format_flags, bool> defaultFlags = std::map<toml::format_flags, bool>{{format_flags::quote_dates_and_times, false}, {format_flags::quote_infinities_and_nans, false}, {format_flags::allow_literal_strings, false}, {format_flags::allow_multi_line_strings, false}, {format_flags::allow_real_tabs_in_strings, false}, {format_flags::allow_unicode_strings, true}, {format_flags::allow_binary_integers, true}, {format_flags::allow_hexadecimal_integers, true}, {format_flags::allow_octal_integers, true}, {format_flags::indent_sub_tables, false}, {format_flags::indentation, true}, {format_flags::relaxed_float_precision, false}, {format_flags::terse_key_value_pairs, false}};
...
std::string parseErrorToString(toml::parse_error e)
{
	...
	return std::string(e.what()) + " ("
	       + (source.begin == source.end ?
	              "at " + sourcePositionToString(source.begin) :
	              "from " + sourcePositionToString(source.begin) + "to " + sourcePositionToString(source.end))
	       + ")";
}
...
Options tableToOptions(sol::optional<sol::table> t)
{
	...
	options.formattedIntsAsUserData = table["formattedIntsAsUserData"].get_or(Options().formattedIntsAsUserData);
	options.temporalTypesAsUserData = table["temporalTypesAsUserData"].get_or(Options().temporalTypesAsUserData);
...
std::optional<std::string> keyToString(sol::object key)
{
	switch (key.get_type())
	{
	case sol::type::string:        return key.as<std::string>(); break;
	case sol::type::number:        return std::to_string(key.as<int64_t>()); break;
	case sol::type::boolean:       return std::string(key.as<bool>() ? "true" : "false"); break;
	case sol::type::none:
	case sol::type::lua_nil:       return "<nil>"; break;
	case sol::type::thread:        return "<thread>"; break;
	case sol::type::function:      return "<function>"; break;
	case sol::type::userdata:      return "<userdata>"; break;
	case sol::type::lightuserdata: return "<userdata>"; break;
	case sol::type::table:         return "<table>"; break;
	case sol::type::poly:          return "<poly>"; break;
	}
...
std::string solLuaDataTypeToString(sol::type type, bool withPrefix)
{
	...
	case sol::type::boolean:       return std::string(withPrefix ? "a " : "") + "boolean";
	case sol::type::function:      return std::string(withPrefix ? "a" : "") + "function";
	case sol::type::lua_nil:       return "nil";
	case sol::type::none:          return "none";
	case sol::type::number:        return std::string(withPrefix ? "a " : "") + "number";
	case sol::type::thread:        return std::string(withPrefix ? "a " : "") + "thread";
	case sol::type::table:         return std::string(withPrefix ? "a " : "") + "table";
	case sol::type::poly:          return "poly";
	case sol::type::string:        return std::string(withPrefix ? "a " : "") + "string";
	default:                       return "none";
	}
}
...
std::variant<int, toml::table *> getTableFromStringInState(sol::state_view state, int index)
{
	...
	try
	{
		...
		{
			return luaL_argerror(L, index, "A string containing a TOML document should be the first argument");
		}
	}
	...
	{
		return luaL_error(state.lua_state(), (std::string("An error occurred during conversion: ") + e.what()).c_str());
	}
}
