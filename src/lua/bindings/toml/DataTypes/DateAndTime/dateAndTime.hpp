...
struct TOMLDate
{
	...
	TOMLDate(uint16_t year, uint8_t month, uint8_t day)
	{
		date = toml::date{year, month, day};
	}

	TOMLDate(toml::date d)
	{
		date = d;
	}
	...
	bool operator==(const TOMLDate& right) const
	{
...
struct TOMLTime
{
	...
	void setNanoSecond(uint32_t nanoSecond)
	{
	...
	bool operator==(const TOMLTime& right) const
	{
...
struct TOMLTimeOffset
{
	...
	TOMLTimeOffset(toml::time_offset tO)
	{
		timeOffset = tO;
	}
	...
	void toTable(sol::table& table)
	{
		table["minutes"] = timeOffset.minutes;
	}

	bool operator==(const TOMLTimeOffset& right) const
	{
		return timeOffset == right.timeOffset;
	}
...
struct TOMLDateTime
{
	...
	TOMLDateTime(TOMLDate d, TOMLTime t) :
	    date(d),
	    time(t){};

	TOMLDateTime(TOMLDate d, TOMLTime t, TOMLTimeOffset tO) :
	    date(d),
	    time(t),
	    timeOffset(tO){};
	...
	TOMLTimeOffset getTimeOffset()
	{
		return timeOffset.value();
	};

	void setDate(TOMLDate d)
	{
		date = d;
	};

	void setTime(TOMLTime t)
	{
		time = t;
	};

	void setTimeOffset(TOMLTimeOffset tO)
	{
		timeOffset = tO;
	};
	...
	toml::date_time asDateTime() const
	{
		...
		if (timeOffset)
		{
			dt.offset = timeOffset.value().timeOffset;
		}
	...
	bool operator==(const TOMLDateTime& right) const
	{
		return asDateTime() == right.asDateTime();
	}
