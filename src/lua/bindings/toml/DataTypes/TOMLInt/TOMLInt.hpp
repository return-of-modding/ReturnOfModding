...
struct TOMLInt
{
	...
	TOMLInt(toml::value<int64_t> tomlInt) :
	    tomlInt(tomlInt){};

	TOMLInt(toml::value<int64_t> tomlInt, toml::value_flags valueFlags) :
	    TOMLInt(tomlInt.get(), static_cast<std::underlying_type<toml::value_flags>::type>(valueFlags))
	{
	}

	TOMLInt(int64_t int64)
	{
		tomlInt = int64;
	}
	...
	void setInt(int64_t int64)
	{
		tomlInt = int64;
	}

	toml::value_flags getFlags() const
	{
		return tomlInt.flags();
	}

	void setFlags(toml::value_flags flags)
	{
		tomlInt.flags(flags);
	}
