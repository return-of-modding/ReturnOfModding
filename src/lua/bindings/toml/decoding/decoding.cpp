...
void insertNodeInTable(sol::table& luaTable, std::variant<std::string, size_t> keyOrIndex, toml::node* node, Options options)
{
	switch (node->type())
	{
	...
	case toml::node_type::date_time:
	{
		...
		if (v.offset.has_value())
		{
			dt.setTimeOffset(TOMLTimeOffset(v.offset.value()));
		}
