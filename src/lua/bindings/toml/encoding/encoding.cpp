...
void unpack(std::variant<std::tuple<toml::table *, std::string>, toml::array *> tableOrArray, std::function<void(toml::table *table, std::string key)> tableFunc, std::function<void(toml::array *array)> arrayFunc)
{
	try
	{
...
void luaValueIntoTOMLTableOrArray(sol::object luaValue, std::variant<std::tuple<toml::table *, std::string>, toml::array *> tableOrArray)
{
	...
	case sol::type::userdata:
	{
		...
		else if (luaValue.is<TOMLInt>())
		{
			...
			unpack(
			    ...
			    [node, &tomlInt](auto array)
			    {
				    array->push_back(*node, tomlInt.flags());
			    });
		}
		break;
	}
	case sol::type::table:
	{
		...
		for (auto &&[key, _] : table)
		{
			...
			else
			{
				...
				throw sol::error((k->empty() ? "The indexes in a table" : std::string("The index") + k.value() + " should be a integer or a string, not " + solLuaDataTypeToString(key.get_type(), true))
				                     .c_str());
			}
		}
...
toml::array *tomlArrayFromLuaArray(sol::table luaArray)
{
	...
	for (auto &&[key, value] : luaArray)
	{
		...
		{
			auto k = keyToString(key);
			throw sol::error((k->empty() ? "The indexes in an array" : std::string("The index ") + k.value() + " should be a integer, not " + solLuaDataTypeToString(key.get_type(), true))
			                     .c_str());
		}
...
toml::table *tomlTableFromLuaTable(sol::table luaTable)
{
	...
	for (auto &&[key, value] : luaTable)
	{
		...
		{
			auto k = keyToString(key);
			throw sol::error((k->empty() ? "The keys in a table" : std::string("The key ") + "\"" + k.value() + "\"" + " should be a string, not " + solLuaDataTypeToString(key.get_type(), true))
			                     .c_str());
		}
